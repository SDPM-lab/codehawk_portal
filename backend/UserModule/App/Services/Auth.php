...
namespace UserModule\App\Services;
...
class Auth
{
     ...
     * user emtity
     *
     * @var \UserModule\UserContext\Entities\User
     */
    protected $user;
    ...
    public function verification(string $token): array
    {
        try {
            ...
            $this->setUser((array)$decoded->data);
            return [
                "statusCode" => "SUCCESS",
                "msg" => "解析成功",
    ...
    /**
     * 取得使用者實體，只有在解析驗證權杖時才能成功取得使用者實體，否則回傳 Null
     *
    ...
    {
        $user = new User($data["first_name"], $data["last_name"], $data["email"], $data["key"]);
        $this->user = $user;
    }
}
